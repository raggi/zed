        ...
        while let Some(transform) = cursor.item() {
            ...
            match &transform.block {
                Some(Block::ExcerptBoundary {
                    ...
                    let matches_start = if *excerpt_controls_after && prev_excerpt.is_some() {
                        start < top_row
                    } else {
                        start <= top_row
                    };
                    ...
                    if matches_start {
                        return next_excerpt.as_ref().map(|excerpt| StickyHeaderExcerpt {
                            ...
                            next_excerpt_controls_present: *excerpt_controls_after,
                            excerpt,
                        });
                    }
                    debug_panic!("conrad thinks this can't happen");
